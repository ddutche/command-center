...
export default function App() {
  ...
  useEffect(() => {
    ...
    updateData((prev) => {
      ...
      if (prev.streak_bumped_today) return prev
  ...
  }, [allDailyDone, streakBumpedToday])
  ...
  const toggleTask = (listKey, id, e) => {
    // Capture position BEFORE React recycles the event
    const rect = e.currentTarget.getBoundingClientRect()
    const emojiPos = { x: rect.left + rect.width / 2, y: rect.top }
    ...
    // Spawn emoji outside the updater using the captured position
    const taskData = (data[listKey] || []).find((t) => t.id === id)
    if (taskData && !taskData.done) {
      const emoji = EMOJI_BURSTS[Math.floor(Math.random() * EMOJI_BURSTS.length)]
      const eid = generateId()
      setFloatingEmojis((prev) => [...prev, { id: eid, emoji, x: emojiPos.x, y: emojiPos.y }])
      setTimeout(() => setFloatingEmojis((prev) => prev.filter((fe) => fe.id !== eid)), 1200)
    }
  }
